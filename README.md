...
The app shows an in-app install prompt when the browser emits
`beforeinstallprompt`, with an iOS fallback hint for Add to Home Screen.
An offline banner appears whenever connectivity is lost.
...
Read-only menu data is cached client-side and may still render while offline,
but loyalty code generation, add coffee, reward redemption, and admin adjustments
